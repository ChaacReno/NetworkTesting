    ...
    public partial class SceneManagerWindow : EditorWindow_UIElements<SceneManagerWindow>, IUIToolkitEditor
    {
        ...
        internal static void Reload()
        {
            ...
            if (window is SceneManagerWindow w && w && !BuildPipeline.isBuildingPlayer)
            {
                ...
                menu?.Q("patch-indicator")?.EnableInClassList("hidden", !PatchUtility.hasUpdate);
        ...
        void ShowMenu()
        {
            ...
            GenericPopup.Open(rootVisualElement.Q("button-menu"), this, alignRight: true, offset: new Vector2(0, -6)).
                Refresh(
                    Item.Create("View patches...", ViewPatches).WithVisibleState(!PatchUtility.hasUpdate),
                    Item.Create("*Patch is available...", ViewPatches).WithVisibleState(PatchUtility.hasUpdate),
                    Item.Separator,
        ...
        void ViewPatches() =>
            Application.OpenURL("https://github.com/Lazy-Solutions/AdvancedSceneManager/releases/latest");

        void OpenDocumentation() => Process.Start("https://lazy-solutions.github.io/AdvancedSceneManager/guides/readme.html");
