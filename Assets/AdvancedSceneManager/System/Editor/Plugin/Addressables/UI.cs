...
namespace AdvancedSceneManager.Plugin.Addressables.Editor
{
    ...
    internal static class UI
    {
        ...
        static void AddScene(string scene)
        {
            ...
            var s = SceneManager.assets.scenes.Find(scene);
            if (!s) throw new Exception("Could not find scene.");
            ...
            _ = settings.
                CreateOrMoveEntry(AssetDatabase.AssetPathToGUID(scene), settings.DefaultGroup, postEvent: false).
                SetLabel("ASM", true, true);

            settings.SetDirty(AddressableAssetSettings.ModificationEvent.EntryCreated, s, postEvent: true, settingsModified: true);
        ...
        static void RemoveScene(string scene)
        {
            ...
            _ = settings.RemoveAssetEntry(AssetDatabase.AssetPathToGUID(scene), postEvent: false);
            settings.SetDirty(AddressableAssetSettings.ModificationEvent.EntryRemoved, s, postEvent: true, settingsModified: true);
